...
//}
Test(sfmm_basecode_suite, malloc_zero_bytes, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void* x = sf_malloc(0);
	cr_assert_null(x, "x is not NULL!");
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
...
Test(sfmm_basecode_suite, free_coalesce_with_both_prev_and_next, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	size_t sz_w = 8, sz_x = 200, sz_y = 300, sz_z = 200, sz_a = 32;
	/* void *w = */ sf_malloc(sz_w);
	void *x = sf_malloc(sz_x);
	void *y = sf_malloc(sz_y);
	void *z = sf_malloc(sz_z);
	...
	assert_quick_list_block_count(0, 0);
	assert_free_block_count(0, 2);
	assert_free_block_count(728, 1);
	assert_free_block_count(3256, 1);

	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}

Test(sfmm_basecode_suite, free_coalesce_with_prev_allocated_but_next_need_coalsce, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	size_t sz_w = 8, sz_x = 200, sz_y = 300, sz_z = 4;
	/* void *w = */ sf_malloc(sz_w);
	void *x = sf_malloc(sz_x);
...
Test(sfmm_basecode_suite, realloc_same_size, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	size_t sz_y = 10, sz_x1 = 10;
	void *x = sf_malloc(sz_y);
	/* void *y = */
	void *r = sf_realloc(x, sz_x1);

	cr_assert_eq(r, x, "x is not equal to r!");
